    ...
    public class ServiceSiteView : TServiceDTO<SiteViewDTO, SiteView, ZZProjectNameZZDBContainer>
    {
        ...
        public void UpdateSiteDefaultView(int siteId, int viewId, bool active)
        {
            ...
            if (active)
            {
                ...
                elementSiteView = Repository.GetStandardQuery(BIA.Net.Model.DAL.AccessMode.Write).Where(x => x.SiteId == siteId).ToList();
        ...
        {
            return Repository.GetStandardQuery(BIA.Net.Model.DAL.AccessMode.Write).Any(x => x.ViewId == viewId);
        }
        ...
        {
            return Repository.GetStandardQuery(BIA.Net.Model.DAL.AccessMode.Write).Where(x => x.ViewId == viewId).Select(x => x.SiteId).Distinct().ToList();
        }
        ...
        {
            return Repository.GetStandardQuery(BIA.Net.Model.DAL.AccessMode.Write).Where(x => x.ViewId == viewId && x.IsDefault).Select(x => x.SiteId).Distinct().ToList();
        }
    }
}