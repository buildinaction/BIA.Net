...
namespace $safeprojectname$.Controllers
{
    ...
    public class SiteController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            siteIndexVM.ListSiteDTO = serviceSite.GetAll(ServiceAccessMode.Write).OrderBy(s => s.Title).ToList();
        ...
        /// <param name="siteIds">sites selected in filter</param>
        /// <returns>ActionResult : _List partial view</returns>
        public ActionResult _List(ICollection<int> memberIds, ICollection<int> siteIds)
        {
            IEnumerable<SiteDTO> listSiteDTO = serviceSite.GetAll(ServiceAccessMode.Write).OrderBy(s => s.Title);
            if (memberIds != null || siteIds != null)
            {
                if (memberIds != null)
                {
                    listSiteDTO = listSiteDTO.Where(s => s.Members.Any(m => memberIds.Contains(m.UserId)));
                }

                if (siteIds != null)
                {
                    listSiteDTO = listSiteDTO.Where(s => siteIds.Contains(s.Id));
                }
            }
        ...
        private void PrepareFilterRelatedLink(SiteIndexVM siteIndexVM)
        {
            ...
            siteIndexVM.Filter.MemberDTOIds = new MultiSelectList(
            siteIndexVM.ListSiteDTO.SelectMany(s => s.Members.Select(m => m.User)).Distinct().ToList(),
            nameof(UserDTO.Id),
            nameof(UserDTO.DisplayName));

            siteIndexVM.Filter.SiteDTOIds = new MultiSelectList(
            siteIndexVM.ListSiteDTO,
            nameof(SiteDTO.Id),
