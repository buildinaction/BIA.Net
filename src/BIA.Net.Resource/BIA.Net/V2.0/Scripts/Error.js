...
function getParameterByName(name, url) {
	if (!url) {
		...
		if (dialog == null) url = window.location.href.replace('#', '');
		    ...
		    url = BIA.Net.Dialog.DialogDiv.GetParentDialogDiv($('MyError')).urlCurrent;
		}
...
(function () {
	...
	if (num === "404") {
		...
		descError.innerText = "Sorry, but the page you are looking for has not been found. Please contact your Helpdesk.";
	}
	else if (num === "403") {
		numError.innerText = "403";
		titleError.innerText = "Forbidden";
		descError.innerText = "You are currently not allowed to access ot this element.";
	}
	else if (num === "401") {
