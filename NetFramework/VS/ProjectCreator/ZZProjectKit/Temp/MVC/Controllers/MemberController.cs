...
namespace $safeprojectname$.Controllers
{
    ...
    public class MemberController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            memberIndexVM.ListMemberDTO = serviceMember.GetAll(ServiceAccessMode.Write).OrderBy(s => s.User.DisplayName).ToList();
        ...
        /// _List Partial View.
        /// </summary>
        /// <param name="memberIds">members selected in filter</param>
        /// <param name="memberRolesIds">member roles selected in filter</param>
        /// <param name="siteId">The site identifier.</param>
        /// <returns>ActionResult : _List partial view</returns>
        public ActionResult _List(ICollection<int> memberIds, ICollection<int> memberRolesIds, int? siteId)
        {
            IEnumerable<MemberDTO> listMemberDTO = serviceMember.GetAllFromSite(siteId).OrderBy(m => m.DisplayName);
            if (memberIds != null || memberRolesIds != null)
            {
                if (memberRolesIds != null)
                {
                    listMemberDTO = listMemberDTO.Where(m => m.MemberRole.Any(mr => memberRolesIds.Contains(mr.Id)));
                }

                if (memberIds != null)
                {
                    listMemberDTO = listMemberDTO.Where(m => memberIds.Contains(m.Id));
                }
            }
        ...
        public ActionResult Edit([Bind(Include = "Id,SiteId,MemberRoleIds")] MemberDTO memberDTO)
        {
            ...
            if (ModelState.IsValid)
            {
                MemberDTO memberUpdated = serviceMember.UpdateValues(memberDTO, new List<string>() { nameof(MemberDTO.MemberRole) });
                RefreshRolesAnotherConnectedMember(memberUpdated);
                return RedirectToAction("Edit", "Site", new { id=memberDTO.SiteId }).Success("Edit was successful");
            }
        ...
        public ActionResult DeleteConfirmed(int id)
        {
            ...
            return RedirectToAction("Edit", "Site", new { id = memberDTO.SiteId }).Success("Delete was successful");
        }
        ...
        private void PrepareRelatedLink(int? excludeSiteId)
        {
            if (excludeSiteId != null)
            {
                ViewBag.UserDTOId = new SelectList(
                    ((ServiceUser)AllServicesDTO.GetService<UserDTO>()).GetAllExcludeSite(excludeSiteId).OrderBy(s => s.DisplayFullName),
                    nameof(UserDTO.Id),
        ...
        private void PrepareFilterRelatedLink(MemberIndexVM memberIndexVM)
        {
            ...
            memberIndexVM.Filter.MemberDTOIds = new MultiSelectList(
            memberIndexVM.ListMemberDTO.Distinct(),
            nameof(UserDTO.Id),
            nameof(UserDTO.DisplayName));

            memberIndexVM.Filter.MemberRoleDTOIds = new MultiSelectList(
