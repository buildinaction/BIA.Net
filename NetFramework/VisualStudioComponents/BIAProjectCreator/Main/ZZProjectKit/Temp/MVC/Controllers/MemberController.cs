...
namespace $safeprojectname$.Controllers
{
    ...
    public class MemberController : BaseController
    {
        ...
        {
            List<MemberDTO> listMemberDTO = serviceMember.GetAll(ServiceAccessMode.Write).OrderBy(s => s.User.DisplayName).ToList();
        ...
        {
            List<MemberDTO> listMemberDTO = serviceMember.GetAll(ServiceAccessMode.Write).OrderBy(s => s.User.DisplayName).ToList();
            return View(PrepareFilterRelatedLink(listMemberDTO));
        }
        ...
        /// Prepares the related link.
        /// </summary>
        /// <param name="listMemberDTO">list of member</param>
        /// <returns>The filter view model</returns>
        private MemberFilterVM PrepareFilterRelatedLink(List<MemberDTO> listMemberDTO)
        {
            MemberFilterVM filterVM = new MemberFilterVM
            {
                MslMember = new MultiSelectList(
                    listMemberDTO.Select(m => m.DisplayName).Distinct()),

                MslMemberRole = new MultiSelectList(
                    listMemberDTO.SelectMany(m => m.MemberRole.Select(mr => mr.Title)).Distinct().ToList())
            };
            return filterVM;
        }
    }
